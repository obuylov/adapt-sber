define([
	...
	var DragndropView = QuestionView.extend({
		...
		setupDragAndDropItems : function () {
			...
			//Below Makes the question text HTML Numbers and tags compatible
			$('.dragquestext').each(function() {
			    var text = $(this).html().replace(/&lt;ul&gt;/g, '<ul class="bulletoff">').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&/g, '').replace(/amp;#39;/g, '\'').replace(/amp;quot;/g, '\"').replace(/nbsp;/g, ' ').replace(/copy;/g, '©').replace(/reg;/g, '®').replace(/pound;/g, '£').replace(/cent;/g, '¢').replace(/yen;/g, '¥').replace(/iquest;/g, '¿').replace(/euro;/g, '€').replace(/trade;/g, '™').replace(/Agrave;/g, 'À').replace(/Aacute;/g, 'Á').replace(/Acirc;/g, 'Â').replace(/Atilde;/g, 'Ã').replace(/Auml;/g, 'Ä').replace(/Aring;/g, 'Å').replace(/AElig;/g, 'Æ').replace(/Ccedil;/g, 'Ç').replace(/Egrave;/g, 'È').replace(/Eacute;/g, 'É').replace(/Ecirc;/g, 'Ê').replace(/Euml;/g, 'Ë').replace(/Igrave;/g, 'Ì').replace(/Iacute;/g, 'Í').replace(/Icirc;/g, 'Î').replace(/Iuml;/g, 'Ï').replace(/ETH;/g, 'Ð').replace(/Ntilde;/g, 'Ñ').replace(/Ograve;/g, 'Ò').replace(/Oacute;/g, 'Ó').replace(/Ocirc;/g, 'Ô').replace(/Otilde;/g, 'Õ').replace(/Ouml;/g, 'Ö').replace(/times;/g, '×').replace(/Oslash;/g, 'Ø').replace(/Ugrave;/g, 'Ù').replace(/Uacute;/g, 'Ú').replace(/Ucirc;/g, 'Û').replace(/Uuml;/g, 'Ü').replace(/Yacute;/g, 'Ý').replace(/agrave;/g, 'à').replace(/aacute;/g, 'á').replace(/acirc;/g, 'â').replace(/atilde;/g, 'ã').replace(/auml;/g, 'ä').replace(/aring;/g, 'å').replace(/aelig;/g, 'æ').replace(/ccedil;/g, 'ç').replace(/egrave;/g, 'è').replace(/eacute;/g, 'é').replace(/ecirc;/g, 'ê').replace(/euml;/g, 'ë').replace(/igrave;/g, 'ì').replace(/iacute;/g, 'í').replace(/icirc;/g, 'î').replace(/iuml;/g, 'ï').replace(/eth;/g, 'ð').replace(/ntilde;/g, 'ñ').replace(/ograve;/g, 'ò').replace(/oacute;/g, 'ó').replace(/ocirc;/g, 'ô').replace(/otilde;/g, 'õ').replace(/ouml;/g, 'ö').replace(/divide;/g, '÷').replace(/oslash;/g, 'ø').replace(/ugrave;/g, 'ù').replace(/uacute;/g, 'ú').replace(/ucirc;/g, 'û').replace(/uuml;/g, 'ü').replace(/yacute;/g, 'ý').replace(/thorn;/g, 'þ').replace(/yuml;/g, 'ÿ').replace(/ndash;/g, '–').replace(/mdash;/g, '—').replace(/lsquo;/g, '‘').replace(/rsquo;/g, '’').replace(/sbquo;/g, '‚').replace(/ldquo;/g, '“').replace(/rdquo;/g, '”').replace(/bdquo;/g, '„').replace(/dagger;/g, '†').replace(/Dagger;/g, '‡').replace(/amp;/g, '');
			    $(this).html(text);
			});
			...
			$items.width(wMax);
			var hMax = this.getMaxHeight($items);

			$droppables.droppable({
				activeClass: "ui-state-active",
				tolerance: "intersect"
			}).height(hMax).css({"line-height": hMax + "px"});

			// MAKES MORE THAN ONE ANSWER MOVE TO THE NEXT LINE
			$('.dragndrop-droppable').each(function(index, elem) {
			     if ( $(elem).is(':nth-child(2)') ) {
			         $(this).css({"margin-left": 350-wMax + "px"});
			     }else{
		...
		getMaxHeight: function($collection) {
			var hMax = 0;
			for (var i = 0; i < $collection.length; i++) {
				var h = $collection.eq(i).height();
				if (h > hMax) hMax = h;
			}
			return hMax + 1;
		},
